<template>
  <transition name="el-zoom-in-top" @after-leave="$emit('dodestroy')">
    <div
      ...
      :style="{ width: isSinglePanel ? '323px' : '646px' }"
    >
      <div class="el-picker-panel__body-wrapper">
        ...
        <div
          ...
          :style="{ minWidth: isSinglePanel ? '258px' : '513px' }"
        >
          <div
            class="
              ...
              is-left
            "
            :style="{ width: isSinglePanel ? '100%' : '50%' }"
          >
            <div class="el-date-range-picker__header">
              <button
                type="button"
                @click="leftPrevYear"
                :disabled="disableYearArrow"
                :class="{ 'is-disabled': disableYearArrow }"
...
export default {
  ...
  data() {
    return {
      ...
      arrowControl: false,
      unlinkPanels: false,
      isSinglePanel: false, //是否单个日面板，是否不能选择重复月份
      disableArrow: false,
    };
  },
  ...
  watch: {
    ...
    defaultValue(val) {
      if (this.isSinglePanel && this.disableArrow) {
  ...
  methods: {
    ...
    handleRangePick(val, close = true) {
      ...
      if (
        !close ||
        (this.isSinglePanel &&
          new Date(val.minDate).getTime() == new Date(val.maxDate).getTime())
      )
        return;
      this.handleConfirm(this.confirmable);
    },
