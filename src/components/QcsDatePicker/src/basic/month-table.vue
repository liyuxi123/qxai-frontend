  ...
  export default {
    ...
    methods: {
      ...
      handleMonthTableClick(event) {
        ...
        let newDate = this.getMonthOfCell(month);
        if (this.selectionMode === 'range') {
            //TODO
          if (!this.rangeState.selecting) {
            if(this.dateLimitOptions?.rangeStart){
                newDate = this.getMonthOfCell(this.dateLimitOptions?.rangeStart - 1);
            }
      ...
      handleMonthClick(month) {
        let newDate = this.getMonthOfCell(month - 1);
        if (this.selectionMode === 'range') {
          if (!this.rangeState.selecting) {
            this.$emit('pick', {minDate: newDate, maxDate: null});
            this.rangeState.selecting = true;
