...
export default {
  ...
  data() {
    return {
      ...
      versioninfo:{"ifupdate":true,"version":''},
  ...
  methods: {
    ...
    async loadData(row) {
      ...
      try {
        ...
        const response = await getDeptNationMetrics(canshu.dataId, canshu.year, canshu.month, canshu.quarter, canshu.stage);
        ...
        if (response.data.code === 200) {
          ...
          this.$message.error(`加载失败/metrics/getDeptNationMetrics：${response.data.message}`);
        }
      } catch (error) {
        // 处理请求失败的情况
        this.error = error;
        console.error("Error getDeptNationMetrics:", error);
        this.$message.error("/metrics/getDeptNationMetrics数据加载失败，请稍后再试。");
      } finally {
    ...
    async searchrMetrics(canshu) {
      ...
      try {
        const response = await getNationMetrics(canshu.year, canshu.month, canshu.quarter, canshu.timeStage);
        if (response.data.code === 200) {
        ...
        // 处理请求失败的情况
        this.error = error;
        this.$message.error("/metrics/getNationMetrics接口请求失败，请检查网络或稍后重试"); // 请求异常时弹出提示
      } finally {
    ...
    async getNationDate() {
      try {
        const response = await getNationDate();
        if (response.data.code === 200) {
          this.resultList = response.data.data.list; // 根据实际 API 响应格式处理数据
          this.dateInfo = response.data.data.default;
        } else {
          this.$message({
            message: '/metrics/getNationDate失败，请重试！',  // 这里可以根据 response.data.message 或其他字段展示具体错误信息
            type: 'error'
          });
        }
      } catch (error) {
        // 处理请求失败的情况
        this.error = error;
        this.$message.error("/metrics/getNationDate接口请求失败失败"); // 请求异常时弹出提示
      }
