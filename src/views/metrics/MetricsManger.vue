...
// },
    ...
    //处理路由过来的指标  不希望将参数直接显示在 URL 中，可以使用 params 替代 query 来传递参数
    getDataRouter(){
    //  console.log("============================")
       if (this.$route.params && Object.keys(this.$route.params).length) {
    //  console.log('this.$route.params.row', this.$route.params.row); // 获取传递的 id
      let canshu = { ...this.$route.params.row };
      canshu.indName = this.$route.params.row.thirdtarget;
      canshu.indId=this.$route.params.row.dataid;
      if (!canshu.author){
         canshu.author= this.user;
      }
      if (!canshu.indId){
